...
modified: 2026-09-16T07:38:48.329Z
---
...
判定问题
判断一个图里面有一个权重<100的环吗？和一个一个图里面没有一个权重<100的环吗？是不同的问题
【P问题】任何一个问题的答案都可以在多项式时间内判定
...
【NP（计算理论定义）】非确定性图灵机可以在poly时间内解决的问题
【NP（应用运筹学定义）】任何问题的yes-实例都可以在多项式时间内验证其答案
==no-实例不行==
【co-NP】NP的补集：可以验证no-实例
【NPC】属于NP，如果有一天NPC的一个问题属于P，那么P=NP
如果存在一个多项式时间内可规约的函数reduction能够让A转换为B，之后如果B有一个Yes的instance那么A就会也有一个Yes的instance，那么B就被称为NP-complete
【NP-hard】NPC及以上难度的问题
exp：可以在指数时间内找到解的问题
【SAT】有没有一种方法判断一个逻辑电路 有没有一种开关方式使得该电路是通的
一种NPC。1971年判定。
使用选择公理解决罗素悖论：设定集合论里面的选择范围
判定一个问题是否是一个NP问题的判定问题是一个NP问题（？）
